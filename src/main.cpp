...
#define GUI_SHOW_IMGAGE 0
#if GUI_SHOW_IMGAGE ==1
...
//        printf("第%d个参数是：%s\r\n",i+1,argv[i]);
//    }
    char *bin_output_path, *img_output_path, *src_path, *temp = nullptr;
    ...
    temp = strstr(argv[0], "\\bin");
//    int index_len = (int)(strlen(argv[0])-strlen(temp));
//    strncpy(bin_output_path,argv[0],index_len);
//    strncpy(img_output_path,argv[0],index_len);
//    strncpy(src_path,argv[0],index_len);
    ...
    for (long i = 0; i < files_counts; i++) {
        if (memcmp(files_list[i].type, "png", 3) == 0 || memcmp(files_list[i].type, "jpg", 3) == 0) {
            ...
            Mat ditherMat = getDitherImgArray(open_buff, header, data, ENCODE_INDEX);
            imwrite(img_output_buff, ditherMat);
//            char *pt = strchr(bin_output_buff,'.');
